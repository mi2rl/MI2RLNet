...
class RegressBoxes(Layer):
    ...
    def __init__(self, mean=None, std=None, input_size=512, anchor_parameters=None, *args, **kwargs):
        """
        ...
        self.anchors = np.expand_dims(
            anchors_for_shape((input_size, input_size), anchor_params=anchor_parameters), axis=0)
    ...
    def call(self, inputs, **kwargs):
        ...
        regression = inputs
        return bbox_transform_inv(self.anchors, regression, mean=self.mean, std=self.std)
...
def filter_detections(
    ...
    if detect_quadrangle:
        alphas = K.gather(alphas, indices)
        ratios = K.gather(ratios, indices)
